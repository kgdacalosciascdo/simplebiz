...
class ReferenceRegistryController extends Controller
{
    ...
    public function lookups(Request $request)
    {
        ...
        return ApiResponse::success(['currencies' => ReferenceCurrency::where('company_id', $companyId)->tap($active)->orderBy('code')->get(['id', 'code', 'name', 'symbol', 'decimal_precision']), 'payment_methods' => PaymentMethod::where('company_id', $companyId)->tap($active)->when($request->filled('direction'), fn ($q) => $request->string('direction')->toString() === 'incoming' ? $q->where('supports_incoming', true) : $q->where('supports_outgoing', true))->orderBy('name')->get(['id', 'code', 'name', 'method_class', 'supports_incoming', 'supports_outgoing']), 'payment_terms' => PaymentTerm::where('company_id', $companyId)->tap($active)->orderBy('name')->get(['id', 'code', 'name', 'term_type', 'due_days']), 'tax_codes' => TaxCode::where('company_id', $companyId)->tap($active)->orderBy('code')->get(['id', 'code', 'name', 'tax_type', 'rate', 'basis']), 'account_titles' => AccountTitle::where('company_id', $companyId)->tap($active)->when($request->filled('classification'), fn ($q) => $q->where('classification', $request->string('classification')))->orderBy('code')->get(['id', 'code', 'name', 'classification', 'normal_balance']), 'expense_categories' => ExpenseCategory::where('company_id', $companyId)->with('accountTitle')->tap($active)->orderBy('name')->get(['id', 'code', 'name', 'account_title_id']), 'branches' => Branch::where('company_id', $companyId)->tap($active)->orderBy('name')->get(['id', 'code', 'name']), 'warehouses' => Warehouse::where('company_id', $companyId)->with('branch')->tap($active)->when($request->filled('branch_id'), fn ($q) => $q->where('branch_id', $request->string('branch_id')))->orderBy('name')->get(['id', 'branch_id', 'code', 'name']), 'stock_locations' => StockLocation::where('company_id', $companyId)->with('warehouse')->tap($active)->when($request->filled('warehouse_id'), fn ($q) => $q->where('warehouse_id', $request->string('warehouse_id')))->orderBy('name')->get(['id', 'warehouse_id', 'parent_id', 'code', 'name']), 'reason_codes' => ReasonCode::where('company_id', $companyId)->tap($active)->when($request->filled('domain'), fn ($q) => $q->where('domain', $request->string('domain')))->orderBy('domain')->orderBy('name')->get(['id', 'code', 'name', 'domain', 'requires_explanation', 'requires_evidence'])]);
    }
