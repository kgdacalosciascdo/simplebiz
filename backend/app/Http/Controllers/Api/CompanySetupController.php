    ...
    public function bootstrap(Request $request)
    {
        ...
        return $this->idempotency->run($request, 'core.setup.bootstrap', null, function () use ($request, $input, $registration) {
            [$user, $company] = DB::transaction(function () use ($request, $input, $registration) {
                ...
                $reportsPermissionIds = Permission::where('module', 'reports')->pluck('id')->all();
                $cashAccountPermissionIds = Permission::where('module', 'cash-accounts')->where(function ($query) {
                    $query->where('key', 'not like', 'cash-accounts.cash-in.%')
                        ...
                        ->where('key', 'not like', 'cash-accounts.statements.%')
                        ->where('key', 'not like', 'cash-accounts.reconciliation.%')
                        ->where('key', 'not like', 'cash-accounts.reconciliations.%')
                        ->where('key', 'not like', 'cash-accounts.reconciliation-adjustments.%')
